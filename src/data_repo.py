...
except ImportError:  # pragma: no cover - avhenger av installert backend
    psycopg = None
...
CREATE TABLE IF NOT EXISTS work_centers (
    ...
    description TEXT NOT NULL DEFAULT '',
    location_code TEXT NOT NULL DEFAULT '',
    labor_cost_hour DOUBLE PRECISION NOT NULL DEFAULT 0,
    machine_cost_hour DOUBLE PRECISION NOT NULL DEFAULT 0,
    overhead_cost_hour DOUBLE PRECISION NOT NULL DEFAULT 0,
    capacity_hours_day DOUBLE PRECISION NOT NULL DEFAULT 0,
    effective_capacity_pct DOUBLE PRECISION NOT NULL DEFAULT 100
);
...
CREATE TABLE IF NOT EXISTS bom_lines (
    ...
    parent_item_no TEXT NOT NULL,
    component_item_no TEXT NOT NULL,
    quantity_per DOUBLE PRECISION NOT NULL DEFAULT 1,
    uom TEXT NOT NULL DEFAULT '',
    scrap_pct DOUBLE PRECISION NOT NULL DEFAULT 0,
    co_product_pct DOUBLE PRECISION NOT NULL DEFAULT 0,
    co_product_item_no TEXT NOT NULL DEFAULT '',
    UNIQUE(parent_item_no, component_item_no)
);
...
CREATE TABLE IF NOT EXISTS routing_lines (
    ...
    run_time_minutes DOUBLE PRECISION NOT NULL DEFAULT 0,
    batch_size DOUBLE PRECISION NOT NULL DEFAULT 1,
    changeover_time_minutes DOUBLE PRECISION NOT NULL DEFAULT 0,
    UNIQUE(item_no, operation_no, work_center_code)
);
...
CREATE TABLE IF NOT EXISTS capacity_days (
    ...
    planned_downtime DOUBLE PRECISION NOT NULL DEFAULT 0,
    UNIQUE(work_center, date)
);
...
def _convert_sql_placeholders(query: str, backend: str) -> str:
    """Konverter enkle SQLite-parametre til valgt backend."""
...
class DataRepo:
    ...
    def connect(self):
        ...
        SQLite er fullt støttet. PostgreSQL er påbegynt og bruker psycopg når
        DATABASE_URL er satt.
        """
        ...
        if self.backend == "postgresql":
            if psycopg is None or psycopg_dict_row is None:
                raise ImportError(
                    "psycopg er ikke installert. Installer requirements.txt før PostgreSQL brukes."
                )
            database_url = os.environ.get("DATABASE_URL", "").strip()
            if not database_url:
                raise ValueError("DATABASE_URL må settes for PostgreSQL-backend.")
            self._conn = psycopg.connect(database_url, row_factory=psycopg_dict_row)
    ...
    def initialize(self):
        ...
        if self.backend == "sqlite":
            try:
                cols = [r["name"] for r in self.execute(
                ...
                if "run_time_minutes" in cols:
                    ...
                    self.execute("DROP TABLE transport_ruter")
                    self.execute("""
            ...
            try:
                self.execute("ALTER TABLE uploaded_files ADD COLUMN comment TEXT NOT NULL DEFAULT ''")
            except sqlite3.OperationalError:
