        ...
        if not data:
            ...
            try:
                ...
                if _comparisons:
                    ...
                    _outputs.append(mo.md("**📋 Detaljer per produkt**"))
                    _outputs.append(mo.md("""
                    Nedenfor vises detaljerte kostnadsdata for hvert enkelt produkt.
                    Tabellene viser materialdetaljer, operasjonsdetaljer og biprodukter.
                    """))

                    for _c in _comparisons:
                        _prod_header = f"**{_c.product_no}** - {_c.product_desc}"
                        _outputs.append(mo.md(_prod_header))
                        ...
                        if _c.simulated_material_details:
                            ...
                            for _md in _c.simulated_material_details:
                                _mat_rows.append({
                                    ...
                                    "Beskrivelse": _md.component_desc,
                                    "Qty per": _md.quantity_per,
                                    "Svinn %": _md.scrap_pct,
                                    "Enhetskost": round(_md.unit_cost, 2),
                                    "Total kost": round(_md.total_cost, 2),
                                })
                            _mat_df = pd.DataFrame(_mat_rows)
                            _outputs.append(mo.md("**🔩 Materialdetaljer:**"))
                            _outputs.append(mo.ui.table(_mat_df, selection=None))
                        ...
                        if _c.simulated_operation_details:
                            ...
                            for _od in _c.simulated_operation_details:
                                _op_rows.append({
                                    ...
                                    "Beskrivelse": _od.operation_desc,
                                    "Arbeidssenter": _od.work_center,
                                    "Run time (min)": _od.run_time_min,
                                    "Setup (min)": _od.setup_time_min,
                                    "Batch": _od.batch_size,
                                    "Kost/time": round(_od.cost_per_hour, 2),
                                    "Run kost": round(_od.run_cost, 2),
                                    "Setup/unit": round(_od.setup_cost_per_unit, 4),
                                    "Total": round(_od.total_cost, 2),
                                })
                            _op_df = pd.DataFrame(_op_rows)
                            _outputs.append(mo.md("**⚙️ Operasjonsdetaljer:**"))
                            _outputs.append(mo.ui.table(_op_df, selection=None))
                        ...
                        if _c.simulated_byproduct_details:
                            ...
                            for _bd in _c.simulated_byproduct_details:
                                _bp_rows.append({
                                    ...
                                    "Beskrivelse": _bd.description,
                                    "Kvantum": _bd.quantity,
                                    "Markedsverdi": round(_bd.market_value, 2),
                                    "Total verdi": round(_bd.total_value, 2),
                                })
                            _bp_df = pd.DataFrame(_bp_rows)
                            _outputs.append(mo.md("**♻️ Biprodukter:**"))
                            _outputs.append(mo.ui.table(_bp_df, selection=None))

                        # Co-produkter (f.eks. B-vare)
                        if _c.co_product_pct > 0 and _c.simulated_co_product_results:
                            _outputs.append(mo.md(f"**🔄 Co-produkt ({_c.co_product_pct}% går til {_c.co_product_item_no}):**"))
                            for _co in _c.simulated_co_product_results:
                                _co_rows = []
                                _co_rows.append({
                                    "Post": "Materialkost",
                                    "Verdi": round(_co.material_cost, 4),
                                })
                                _co_rows.append({
                                    "Post": "Operasjonskost",
                                    "Verdi": round(_co.operation_cost, 4),
                                })
                                _co_rows.append({
                                    "Post": "Setupkost",
                                    "Verdi": round(_co.setup_cost, 4),
                                })
                                _co_rows.append({
                                    "Post": "Brutto",
                                    "Verdi": round(_co.gross_production_cost, 4),
                                })
                                if _co.by_product_value > 0:
                                    _co_rows.append({
                                        "Post": "Biproduktverdi",
                                        "Verdi": -round(_co.by_product_value, 4),
                                    })
                                _co_rows.append({
                                    "Post": "Netto",
                                    "Verdi": round(_co.net_production_cost, 4),
                                })
                                _co_df = pd.DataFrame(_co_rows)
                                _outputs.append(mo.md(f"**{_co.product_no} - {_co.product_desc}**"))
                                _outputs.append(mo.ui.table(_co_df, selection=None))
                            
                            # Sammenlign A-vare med og uten co-produkt
                            if _c.original_co_product_results:
                                _orig_co = _c.original_co_product_results[0]
                                _sim_co = _c.simulated_co_product_results[0]
                                _cmp_rows = []
                                _cmp_rows.append({
                                    "Produkt": _c.product_no,
                                    "Org. netto": round(_c.original_net_cost, 4),
                                    "Sim. netto": round(_c.simulated_net_cost, 4),
                                    "Diff": round(_c.net_diff, 4),
                                })
                                _cmp_rows.append({
                                    "Produkt": _orig_co.product_no,
                                    "Org. netto": round(_orig_co.net_production_cost, 4),
                                    "Sim. netto": round(_sim_co.net_production_cost, 4),
                                    "Diff": round(_sim_co.net_production_cost - _orig_co.net_production_cost, 4),
                                })
                                _cmp_df = pd.DataFrame(_cmp_rows)
                                _outputs.append(mo.md("**📊 Sammenligning A-vare vs B-vare:**"))
                                _outputs.append(mo.ui.table(_cmp_df, selection=None))

                        # Scenariototal for dette produktet
                        if _c.planned_quantity:
                            _outputs.append(mo.md(
                                f"**📦 Scenariototal:** {_c.planned_quantity:,.0f} enheter → "
                                f"Total netto: {_c.simulated_total_net_cost:,.2f} | "
                                f"Kost/enhet: {_c.simulated_cost_per_unit:.2f} | "
                                f"Timebehov: {_c.simulated_total_hours:.2f} timer"
                            ))
